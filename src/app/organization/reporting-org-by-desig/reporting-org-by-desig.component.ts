...
@Component({
  selector: 'app-reporting-org-by-desig',
  templateUrl: './reporting-org-by-desig.component.html',
  providers: [ORGStructureModelService, OrgStructureDataService],
  standalone:true,
  imports:[SharedModule,HighchartsChartModule]
})
export class ReportingOrgByDesigComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    try {
      ...
      this.getReportingOrgByDesignation();
    } catch (e) {
      this.showErrorMsg(e);
    }
  }
  ...
  getReportingOrgByDesignation() {
    try {
      this.modelSvc.spData.pageNo = 0;
      this.dataSvc.getReportingOrgByDesignation(this.modelSvc.spData).subscribe({
        next: (res: any) => {
