...
loadVoucherToEdit(){
  try {    
    ...
    if(!this.voucherModel.isMultiEntry && this.voucherModel.voucherTitleCd==FixedIDs.voucherTitleCd.debitVoucherBank.code){ 
      ...
      if(this.voucherModel.voucherItemList.length > 0)
      {
        this.utilitySvc.deletedDataPushInDdl(this.voucherModel.voucherItemList[0].chequeLeafID, this.voucherModel.voucherItemList[0].chequeLeafNo, 'id', 'name', this.modelSvc.chequeLeafNoList);
      }
    } 
