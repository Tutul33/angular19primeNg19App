...
@Injectable()
export class BankBookModelService {
  keyValuePair: any;
    ...
    summaryData(cashBookList:any) {
      try {
        if (cashBookList.length > 0) {
            ...
            this.fromBank = this.bankList.find(x => x.id == this.searchParam.bankLedgerID).ledger;
  ...
  bankBookValidation(): any {
    return {
      bankBookValidateModel: {
        ...
        // },
        bankLedgerID: {
          required: GlobalConstants.validationMsg.bankLedgerID,
        },
        fromDate: {
