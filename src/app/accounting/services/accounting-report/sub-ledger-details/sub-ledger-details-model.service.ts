...
@Injectable()
export class SubLedgerDetailsModelService {
  keyValuePair: any;
  ...
  formValidation(): any {
    return {
      formValidateModel: {
        ...
        subLedgerDetailID: {
          required: GlobalConstants.validationMsg.subLedgerDetailID,
        },
        fromDate: {
  ...
  setNewSearchParam() {
    try {
      ...
      this.searchParam = {
        ...
        subLedgerDetailID:this.subLedgerId,
  ...
  summaryData(cashBookList:any) {
    try {
      if (cashBookList.length > 0) {
          ...
          this.fromBank = this.ledgerList.find(x => x.id == this.searchParam.ledgerID).ledger;
